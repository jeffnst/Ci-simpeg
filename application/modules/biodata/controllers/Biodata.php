...
function __construct()
	{
		parent::__construct();
		$this->load->model('Biodata_model','Biomodel');
	}
		...
		if(!empty($cek) && $stts=='01')
			{
				...
				$data['ptitle'] = '';
				$data['infobar'] = '';
				$data['konten'] = 'biodata/view_biodata';
				$data['skrip']='biodata/biodata_skrip';
...
public function biodata_list()
	{


		$list = $this->Biomodel->get_datatables();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $person) {
			...
			if($person->foto)
				$row[] = '<a href="'.base_url('foto/'.$person->foto).'" target="_blank"><img src="'.base_url('foto/'.$person->foto).'" class="img-responsive" style="width:75px;height:120px"/></a>';
			else
				$row[] = '(No photo)';

			//add html for action
			$row[] = '<a class="btn btn-xs btn-raised btn-primary " href="javascript:void(0)" title="Edit" onclick="edit_person('."'".$person->uid."'".')"><i class="glyphicon glyphicon-pencil"></i> </a>
					<a class="btn btn-xs btn-raised btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_person('."'".$person->uid."'".')"><i class="glyphicon glyphicon-trash"></i> </a>';
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->Biomodel->count_all(),
						"recordsFiltered" => $this->Biomodel->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->Biomodel->get_by_id($uid);
	...
	public function biodata_add()
	{
		...
		$insert = $this->Biomodel->save($data);
	...
	public function biodata_update()
	{
		...
		if(!empty($_FILES['foto']['name']))
		{
			...
			//delete file
			$person = $this->Biomodel->get_by_id($this->input->post('uid'));
		...
		$this->Biomodel->update(array('uid' => $this->input->post('uid')), $data);
		echo json_encode(array("status" => TRUE));
	}
	...
	{
		//delete file
		$person = $this->Biomodel->get_by_id($uid);
		if(file_exists('foto/'.$person->foto) && $person->foto)
			unlink('foto/'.$person->foto);

		$this->Biomodel->delete_by_id($uid);
		echo json_encode(array("status" => TRUE));
	}
